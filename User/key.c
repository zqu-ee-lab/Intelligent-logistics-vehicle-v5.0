...
static void bsp_InitKeyVar(void)
{
	...
	/* 组合键 */
	s_tBtn[3].IsKeyDownFunc = IsKeyDown9;
	s_tBtn[4].IsKeyDownFunc = IsKeyDown10;
}
...
static void bsp_DetectKey(uint8_t i)
{
	...
	else
	{
		if (pBtn->State == 1)
		{
			if (++pBtn->UpTime > KEY_FILTER_TIME)
			{
				pBtn->State = 0;
				pBtn->UpTime = 0;
				// 判断是否长按
				if (pBtn->Count > pBtn->LongTime)
				{
